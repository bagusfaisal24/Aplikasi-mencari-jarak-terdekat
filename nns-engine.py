...
mycursor.execute("SELECT * FROM sekolah")
...
for i in range (0,len(points)):
    tempout = KDTree(points).closest_point(now)
    route.append(tempout)
    now = tempout
    points.remove(now)
    
route = [temppoints.index(x) for x in route]
route = [myresult[x][1] for x in route]
